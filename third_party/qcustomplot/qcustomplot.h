...
extern const QMetaObject staticMetaObject; // in moc-run we create a static meta object for QCP "fake"
                                           // object. This line is the link to it via
  ...
  } else // don't need to prepend, so append and merge if necessary
  {
      ...
      if (oldSize > 0 &&
          !qcpLessThanSortKey<DataType>(*(constEnd() - n - 1),
                                        *(constEnd() - n))) // if appended range keys aren't all greater than
...
void QCPDataContainer<DataType>::performAutoSqueeze()
{
  ...
  {
      shrinkPostAllocation = postAllocSize > usedSize * 1.5; // QVector grow strategy is 2^n for static
...
public:
  ...
  virtual double selectTest(const QPointF &pos, bool onlySelectable,
      ...
      Q_DECL_OVERRIDE = 0; // actually introduced in QCPLayerable as non-pure, but
...
protected:
  ...
  QList<QCPGraph *>            mGraphs; // extra list of plottables also in mPlottables that
                                        // are of type QCPGraph
    ...
    if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) && painter->pen().style() == Qt::SolidLine &&
        ...
        while (i < lineDataSize) {
            if (qIsNaN(lineData.at(i).y()) || qIsNaN(lineData.at(i).x()) ||
                qIsInf(lineData.at(i).y())) // NaNs create a gap in the line. Also filter
